...
float duration(Indi v, int i) {
	float c = zz;
	float f = 1, f_;

	float x = v.task[i].sched;
	...
	if (x > c - dur[i]) {
		...
		f_ = 0.8 + 0.2 * (c - dur[i] - x) / (c - dur[i]);
		f = min(f, f_);
	}

	for (int j : adj_b[i]) {
		f_ = 1;
		float a = v.task[j].sched;

		if (x < a + dur[j]){
			f_ = min(f_, (x - a) / dur[j]);
		}
		if (x > c - dur[i]) {
			float ff = 0.8 * (c - x) / dur[i];
			f_ = min(f_, ff);
		}

		if (x >= a + dur[j] && x <= c - dur[i]) {
			float ff = 0.8 + 0.2 * (c - dur[i] - x) / (c - dur[i] - a - dur[j]);
			f_ = min(f_, ff);
		}
...
float assignment(Indi a, int g) {
	...
	for(int k = 0; k < r; k++) {
		...
		for(int i = 0; i < t - 1; i++) {
			for(int j = i + 1; j < t; j++) {
				if (a.task[i].assig[k] && a.task[j].assig[k]) {
...
Task cross(Task a, Task b) {
	...
	int y = x + rand() % (r - x);
	for(int i = 0; i < r; i++) {
		if (i >= x && i <= y) d.assig[i] = e.assig[i];
	}
	return d;
